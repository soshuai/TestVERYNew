package com.example.veryw.testveryssssss.fonsview;
...
public class ContinueFTP {
     ...
     * @throws IOException
     */
    public DownloadStatus download(String remote, String local, ProgressListener plistener) throws IOException {
        // 设置被动模式
     ...
     * @throws IOException
     */
    public UploadStatus upload(String local, String remote, ProgressListener plistener) throws IOException {
        // 设置PassiveMode传输
        ftpClient.enterLocalPassiveMode();
        // 设置以二进制流的方式传输
        ftpClient.setFileType(FTP.BINARY_FILE_TYPE);
        ...
        if (files.length == 1) {
            ...
            // 尝试移动文件内读取指针,实现断点续传
            result = uploadFile(remoteFileName, f, ftpClient, remoteSize, plistener);
            ...
            if (result == UploadStatus.Upload_From_Break_Failed) {
                ...
                result = uploadFile(remoteFileName, f, ftpClient, 0, plistener);
            }
        } else {
            result = uploadFile(remoteFileName, new File(local), ftpClient, 0, plistener);
        }
        return result;
    }
    ...
    public UploadStatus uploadFile(String remoteFile, File localFile, FTPClient ftpClient, long remoteSize,
            ProgressListener plistener) throws IOException {
        UploadStatus status;
        // 显示进度的上传
